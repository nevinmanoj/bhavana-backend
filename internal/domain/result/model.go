...
// written once per entry when an event is finalized.
type Result struct {
	ID         int64     `db:"id"`
...
// EntryTotal is an entry's aggregate score for an event, the input to Rank.
...
// EventResult is one row of a finalized event's results, joined with entry
...
// LeaderboardRow is one entry's mapped result joined with its school and
...
// LeaderboardEventBreakdown is one entry's placing within one event, nested
...
// points across every event it placed in, and the per-entry breakdown.
...
// UnscoredEntry is an entry with zero score rows for the event, used by the
// finalize-readiness check.
type UnscoredEntry struct {
	EntryID     int64  `db:"entry_id" json:"entry_id"`
	ChestNumber *int64 `db:"chest_number" json:"chest_number"`
	SchoolName  string `db:"school_name" json:"school_name"`
}
...
type FinalizeReadiness struct {
	...
	UnscoredEntries []UnscoredEntry `json:"unscored_entries"`
