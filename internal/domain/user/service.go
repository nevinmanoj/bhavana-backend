...
type UserService interface {
	CreateUser(ctx context.Context, email, password, name, jwtToken string, role core.UserRole) (*User, error)
...
func (s *userService) CreateUser(ctx context.Context, email, password, name, jwtToken string, role core.UserRole) (*User, error) {
	if role == core.RoleAdmin {
		if jwtToken == "" {
			return nil, fmt.Errorf("Unauthorized: JWT token is required to create admin users")
		}
		claims, err := auth.ParseToken(jwtToken, s.jwtSecret)
		if err != nil {
			return nil, fmt.Errorf("Unauthorized")
		}
		if claims.Role != core.RoleAdmin {
			return nil, fmt.Errorf("Forbidden: Only admins can create admin users")
		}
	}
	return s.repo.CreateUser(ctx, email, password, name, role)
}
